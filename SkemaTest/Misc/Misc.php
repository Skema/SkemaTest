<?php
namespace SkemaTest\Misc;

use Testify\Testify;
use Skema\Field;
use Skema\Set;

class Misc extends \SkemaTest\BaseTest {
	public static function run(Testify $tf) {
		...
		$tf->test('Longitude and latitude', function(Testify $tf) use ($vendorDir) {
			$longitude = -81.581211;
			$latitude = 28.418749;

			require_once $vendorDir . 'skema/skema/examples/longitudeLatitude.php';
		...
		$tf->test('Complete Address', function(Testify $tf) use ($vendorDir) {

			require $vendorDir . 'skema/skema/examples/simpleAddress.php';
		...
		$tf->test('Markup (wikiLingo)', function(Testify $tf) use ($vendorDir) {

			require_once $vendorDir . 'skema/skema/examples/wikiMarkup.php';
						...
						switch ($key) {
							...
							case 'favoritecolor':
								...
								foreach ($options as $i => $option) {
									switch ($i) {
										case 0:
											$tf->assertEquals($option->textContent, 'blue', 'Correct items in select element');
											break;
										case 1:
											$tf->assertEquals($option->getAttribute('selected'), 'selected', 'Correct item selected');
											$tf->assertEquals($option->textContent, 'red', 'Correct value selected');
											break;
										case 2:
											$tf->assertEquals($option->textContent, 'orange', 'Correct items in select element');
											break;
									}
								}
								break;
